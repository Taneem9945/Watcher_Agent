...
def main():
    ...
    max_examples = int(config["context_builder"]["num_context_examples"])
    with torch.no_grad():
        for window_id, (xb, yb) in enumerate(test_loader):
            xb = xb.to(device)
            ...
            for i in range(xb_np.shape[0]):
                ...
                prev_packet = packet
                prev_signal = signal_packet
                if len(packets) >= max_examples:
                    break
            if len(packets) >= max_examples:
                break

    ensure_dir("results")
    with open("results/llm_context_examples.json", "w", encoding="utf-8") as f:
        json.dump(packets, f, indent=2)
    print("results/llm_context_examples.json")
